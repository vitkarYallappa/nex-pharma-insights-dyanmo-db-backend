"""
Project Modules Statistics Repository - Uses simplified base repository with only 4 methods
...
class ProjectModulesStatisticsRepository(BaseRepository):
    """Project modules statistics repository with 4 essential methods"""
    
    def __init__(self):
        super().__init__(ProjectModulesStatisticsModel.table_name())
    
    async def create(self, statistics_model: ProjectModulesStatisticsModel) -> ProjectModulesStatisticsModel:
        ...
        return ProjectModulesStatisticsModel.from_dict(created_data)
    
    async def find_one_by_query(self, query: Dict[str, Any]) -> Optional[ProjectModulesStatisticsModel]:
        """Find project modules statistics by query"""
        statistics_data = await super().find_one_by_query(query)
        return ProjectModulesStatisticsModel.from_dict(statistics_data) if statistics_data else None
    
    async def find_all_by_query(self, query: Optional[Dict[str, Any]] = None, limit: Optional[int] = None) -> List[ProjectModulesStatisticsModel]:
